...
modelos_sac = {
    ...
    # --- ENCERRAMENTO DE CHAT ADICIONADO ---
...
def registrar_e_limpar(setor, texto_pronto):
    ...
    if sucesso:
        ...
        # --- LÓGICA ATUALIZADA DE LIMPEZA ---
        # Reseta APENAS campos manuais (texto), preservando as seleções (selectbox)
        campos_para_limpar = [f"cliente{sufixo}", f"nf{sufixo}", f"ped{sufixo}"]
        
        if setor == "SAC":
            campos_para_limpar.extend([
                "end_coleta_sac", "fab_in_7", "cont_assist_in_7", 
                "data_comp_out_7", "nf_out_7", "link_out_7", "cod_post_sac", 
                "data_ent_sac", "fab_glp", "site_glp", "val_desc", "prev_ent", 
                "link_rast", "nf_rast", "rua_ins", "cep_ins", "num_ins", 
                "bair_ins", "cid_ins", "uf_ins", "comp_ins", "ref_ins", 
                "data_limite_recusa", "data_entrega_canc_ent"
            ])
...
def pagina_pendencias():
    ...
    if tipo_fluxo == "Pendência":
        ...
        with st.form("form_atraso", clear_on_submit=True): # clear_on_submit APENAS limpa inputs de texto por padrão, selects podem resetar ou manter dependendo da implementação interna do streamlit, aqui forçamos reset total.
            c1, c2 = st.columns(2)
            with c1:
...
def pagina_sac():
    ...
    if opcao not in excecoes_nf and opcao not in scripts_martins:
        ...
        if "\n" in texto_base:
            ...
            # Lógica específica para Solicitação de Coleta
